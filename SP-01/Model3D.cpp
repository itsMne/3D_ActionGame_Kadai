...
HRESULT Model3D::InitModel(const char* ModelPath)
{
	...
	Scale = XMFLOAT3(INITIAL_SCALE, INITIAL_SCALE, INITIAL_SCALE);
	// FBXファイルの読み込み
...
HRESULT Model3D::InitModel(int ModelPath)
{
	...
	Position = XMFLOAT3(0.0f, -20.0f, 0.0f);
	Rotation = XMFLOAT3(0.0f, 0.0f, 0.0f);
	//Rotation = XMFLOAT3(0.0f, XM_PI, 0.0f);//DEL
	Scale = XMFLOAT3(INITIAL_SCALE, INITIAL_SCALE, INITIAL_SCALE);
	// FBXファイルの読み込み
	if (!Models[ModelPath]) {
...
void Model3D::SwitchAnimation(int nNewAnimNum, int FrameSlowness, float AnimationFrameSkip)
{
	if (nCurrentAnimation == nNewAnimNum) {
		...
		nInitialFrame = g_pModel->GetInitialAnimFrame();
...
{
#if STOP_ANIMATION
	static int nStopAnimCounter = 0;
	if (++nStopAnimCounter > 1) {
		nStopAnimCounter = 1;
		return;
	}
#endif
